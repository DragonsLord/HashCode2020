...
{
    /// <summary>
    /// lib: total possible scores
    /// lib: scan plan: books scan each day
    /// 
    /// 1 step :
    /// choose lib which will produce most scrore for all days
    /// 
    /// 2 step (after 1 lib sign up) :
    /// exclude already scanned books
    /// repeat step 1 logic
    /// ??? how to deal with duplicate books (probably include that when creating scan plan for library as well)
    /// 
    /// x
    /// 1   2   3
    /// | \ |\  |\
    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            //var input = Parser.ParseInput($"inputs/{currFile}.txt");

            //var output = Resolve(input);

            //Parser.WriteOutput(output, $"{currFile}_result.txt");
